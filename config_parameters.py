...
ENHANCED_TEXT_PATTERNS = {
    ...
    'crush_resistance': r'Max\.\s*Crush\s*Resistance\s*([\d.]+\s*N/\d+\s*cm)',
    'impact_resistance': r'Impact\s*Resistance\s*([\d.]+\s*N\.m)',
    'torsion': r'Torsion\s*(±\s*\d+\s*°)',
    'minimum_bend_radius': r'Minimum\s*Bend\s*Radius\s*([\d.]+\s*x\s*D)',
    'water_penetration': r'Water\s*Penetration\s*Test\s*([\d.]+\s*m\s*water\s*head,\s*[\d.]+\s*m\s*sample,\s*[\d.]+\s*hours)',
    'installation_temp': r'Installation\s+([-+]?\d+\s*°C\s*to\s*[+-]?\d+\s*°C)',
    'operation_temp': r'Operation\s+([-+]?\d+\s*°C\s*to\s*[+-]?\d+\s*°C)',
